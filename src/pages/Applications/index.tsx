...
const statusConfig: Record<ApplicationStatus, { label: string; color: string }> = {
  ...
  offered: { label: '已 Offer', color: 'green' },
}
...
export default function ApplicationsPage() {
  ...
  const cities = useMemo(() => Array.from(new Set(applications.map((a) => a.city).filter(Boolean))) as string[], [applications])

  const filteredApps = useMemo(
    () =>
      applications.filter((app) => {
        const matchSearch =
          !searchText || app.company.toLowerCase().includes(searchText.toLowerCase()) || app.position.toLowerCase().includes(searchText.toLowerCase())
    ...
    [applications, cityFilter, jobTypeFilter, searchText, statusFilter],
  )
  ...
  const columns = [
    {
      ...
      dataIndex: 'company',
      key: 'company',
      render: (text: string, record: Application) => <a onClick={() => setSelectedAppId(record.id)}>{text}</a>,
    },
    ...
    },
    {
      ...
      dataIndex: 'status',
      key: 'status',
      render: (status: ApplicationStatus) => <Tag color={statusConfig[status].color}>{statusConfig[status].label}</Tag>,
    },
  ]
  ...
  const saveEdit = async () => {
    if (!selectedApp) return
    await withLoading(async () => {
      await updateApplication(selectedApp.id, editValues)
  ...
  const handleAddAssessment = async (values: any) => {
    ...
    await withLoading(async () => {
      await addAssessment(selectedApp.id, {
        name: values.name,
    ...
    }).catch((err: unknown) => message.error(err instanceof Error ? err.message : '新增测评失败'))
  }
  ...
  const handleAddInterview = async (values: any) => {
    ...
    const datetime = dayjs(values.date).hour(values.time.hour()).minute(values.time.minute())
    await withLoading(async () => {
      await addInterview(selectedApp.id, {
        round: values.round,
    ...
    }).catch((err: unknown) => message.error(err instanceof Error ? err.message : '新增面试失败'))
  }

  const EditActions = ({ section }: { section: 'basic' | 'jd' }) =>
    editingSection === section ? (
      <Space size="small">
        <Button size="small" type="primary" icon={<SaveOutlined />} onClick={() => void saveEdit()} loading={submitting}>
          保存
        </Button>
        <Button size="small" icon={<CloseOutlined />} onClick={cancelEdit} disabled={submitting}>
          取消
        </Button>
      </Space>
    ) : (
      <Button type="link" size="small" icon={<EditOutlined />} onClick={() => startEdit(section)}>
        编辑
      </Button>
    )
  ...
  return (
    <div>
      ...
      <Drawer title={selectedApp ? `${selectedApp.company} · ${selectedApp.position}` : ''} size="large" open={!!selectedApp} onClose={() => setSelectedAppId(null)}>
        {selectedApp && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
              ...
              <Descriptions column={2} size="small" bordered>
                ...
                <Descriptions.Item label="岗位分类">
                  {editingSection === 'basic' ? (
                    <Select
                      ...
                      onChange={(v) => setEditValues((p) => ({ ...p, jobType: v }))}
                ...
                <Descriptions.Item label="投递时间">
                  {editingSection === 'basic' ? (
                    <DatePicker
                      size="small"
                      value={editValues.applyDate ? dayjs(editValues.applyDate) : undefined}
                      onChange={(d) => setEditValues((v) => ({ ...v, applyDate: d?.format('YYYY-MM-DD') }))}
                    />
                  ) : (
                  ...
                  <Select
                    ...
                    onChange={(v) => {
                      void withLoading(async () => {
                        await updateStatus(selectedApp.id, v)
                      }).catch((err: unknown) => message.error(err instanceof Error ? err.message : '更新状态失败'))
                    }}
              ...
              <Descriptions column={1} size="small" bordered>
                ...
                <Descriptions.Item label="岗位JD">
                  {editingSection === 'jd' ? (
                    ...
                    <div style={{ whiteSpace: 'pre-wrap', fontSize: 13, maxHeight: 120, overflow: 'auto' }}>{selectedApp.jd}</div>
                  ) : (
                    '-'
                  )}
                </Descriptions.Item>
            ...
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                ...
                <Space>
                  <Button type="link" size="small" onClick={() => navigate('/assessments')}>
                    查看全部 ↗
                  </Button>
                  <Button size="small" onClick={() => setAddAssessmentOpen(true)}>
                    + 新增测评
                  </Button>
                </Space>
              </div>
              {selectedApp.assessments.length === 0 ? (
                ...
                <List
                  ...
                  renderItem={(a) => (
                    <List.Item extra={<Button type="link" size="small" onClick={() => navigate('/assessments')}>↗</Button>}>
                      <List.Item.Meta
                        title={
                          <Space>
                            {a.name}
                            <Tag color={a.status === 'done' ? 'green' : a.status === 'expired' ? 'default' : 'gold'}>
                              {a.status === 'done' ? '已完成' : a.status === 'expired' ? '已失效' : '待完成'}
                            </Tag>
                          </Space>
                        }
            ...
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                ...
                <Space>
                  ...
                  <Button size="small" onClick={() => setAddInterviewOpen(true)}>
                    + 新增面试
                  </Button>
                </Space>
              </div>
              {selectedApp.interviews.length === 0 ? (
                ...
                <List
                  ...
                  renderItem={(i) => (
                    <List.Item extra={<Button type="link" size="small" onClick={() => navigate('/interviews')}>↗</Button>}>
                      <List.Item.Meta
                        title={
                          <Space>
                            <Tag color="purple">{i.round}</Tag>
                            <Tag color={i.status === 'done' ? 'green' : i.status === 'abandoned' ? 'default' : 'blue'}>
                              {i.status === 'done' ? '已完成' : i.status === 'abandoned' ? '已放弃' : '待面试'}
                            </Tag>
                          </Space>
                        }
            ...
            <div>
              ...
              <Input.TextArea
                ...
                onBlur={(e) => {
                  void withLoading(async () => {
                    await updateApplication(selectedApp.id, { note: e.target.value })
                  }).catch((err: unknown) => message.error(err instanceof Error ? err.message : '保存备注失败'))
                }}
              />
            </div>
            ...
            <Button
              ...
              onClick={() => {
                modal.confirm({
                  title: '确认删除',
                  content: '删除后该投递下的测评和面试记录也会一并删除，且不可恢复。',
                  okText: '确认删除',
                  okType: 'danger',
                  cancelText: '取消',
                  onOk: async () => {
      ...
      >
        <Form form={addForm} layout="vertical" onFinish={(values) => void handleAddApplication(values)}>
          <Form.Item name="company" label="公司名称" rules={[{ required: true, message: '请填写公司名称' }]}>
            <Input placeholder="如：字节跳动" />
          </Form.Item>
          <Form.Item name="position" label="岗位名称" rules={[{ required: true, message: '请填写岗位名称' }]}>
          ...
          <Form.Item name="resumeId" label="关联简历">
            <Select
              allowClear
              placeholder="选择简历（可选）"
              options={resumes.map((r) => ({ label: `${r.name}${r.isDefault ? '（默认）' : ''}`, value: r.id }))}
            />
          </Form.Item>
      ...
      <Modal
        title={`新增测评 · ${selectedApp?.company || ''}`}
        ...
        confirmLoading={submitting}
      >
        <Form form={assessmentForm} layout="vertical" onFinish={(values) => void handleAddAssessment(values)}>
          ...
          <Form.Item name="platform" label="测评平台">
            <Input placeholder="如：北森、智鼎" />
          </Form.Item>
          <Form.Item name="link" label="测评链接">
            <Input placeholder="https://..." />
          </Form.Item>
      ...
      <Modal
        title={`新增面试 · ${selectedApp?.company || ''}`}
        ...
        confirmLoading={submitting}
      >
        <Form form={interviewForm} layout="vertical" onFinish={(values) => void handleAddInterview(values)}>
          <Form.Item name="round" label="面试轮次" rules={[{ required: true, message: '请填写面试轮次' }]}>
            <div>
              <Space size="small" style={{ marginBottom: 8, flexWrap: 'wrap' }}>
                {roundPresets.map((r) => (
                  <Button key={r} size="small" onClick={() => interviewForm.setFieldValue('round', r)}>
                    {r}
                  </Button>
                ))}
              </Space>
          ...
          <Form.Item name="format" label="面试形式">
            <Select
              ...
              options={[
                ...
                { label: '线上 - Zoom', value: 'online_zoom' },
